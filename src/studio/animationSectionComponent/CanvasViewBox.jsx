...
const CanvasViewBox = () => {
    ...
    const emojiArray = ['✨','🧨','🤿','🪄','🎲','🧩','🎧','🎷','⛓️','💻'];
    useEffect(() => {
      if (canavsLoaded) return;

      const interval = setInterval(() => {
        const randomIndex = Math.floor(Math.random() * emojiArray.length);
        setEmoji(emojiArray[randomIndex]);
      }, 1000);
    ...
    useEffect(() => {
      if(open == false)
      {
        updateSpeakerList(projectId,speakerList); 
        ...
        return (
            <div style={{display:'flex',flexDirection:'column',height:'100%',alignItems:'center',justifyContent:'center'}}>
                ...
                <Typography style={{height: '25px', fontFamily:'karma', fontSize:'14px',padding:'10px',fontWeight:'600'}}>
                    Your Video is Being Created {emoji}
                </Typography>
            </div> 
        )
    }
          ...
          { experienceLoaded ?<Experience/> : <Loader/>}
                ...
                BackdropProps={{
                  ...
                  {/* </Grid2><Grid2 style={styles.AvatarSelectorModal}> */}
                      <AvatarSelector speakerList={speakerList} setSpeakerList={setSpeakerList} setIsOpen={setOpen} disabled={true}/>
                  </Grid2>
              </Modal>
        </>
      );
